...
import { studioCapabilities } from "../src/web/studio.js";
...
async function harness(): Promise<Harness> {
  ...
  const studio: StudioApiOptions = { database, keys, config, dataPort: config.dataPort, blobs: true };
  const server = createServer(
    webHandler(() => ({ ...DISCOVERY, capabilities: studioCapabilities(studio) }), {
      studio,
