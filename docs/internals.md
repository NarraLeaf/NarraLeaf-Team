...
How the parts of a NarraLeaf Team server work. None of it is needed to run one:
that is [Deployment](operations.md), and what Team is, above the parts, is
...
megabytes, and that is not an accident to be optimized away: scrypt at OWASP's
...
## The session, and what it replaced
...
included. That is authorship rather than authorization.
...
## Instances, rooms and attachments
...
[Message catalogues](#message-catalogues).
